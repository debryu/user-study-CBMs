...
\midrule
Micro avg    & 0.87 & 0.80 & 0.83 & 1{,}832 \\
Macro avg    & 0.82 & 0.58 & 0.63 & 1{,}832 \\
Weighted avg & 0.86 & 0.80 & 0.81 & 1{,}832 \\
\bottomrule
\end{tabular}
\caption{Emails concept extractor: per-concept classification report on the
1{,}000-row test set (multi-label; support is the number of positive
instances per concept, not the number of test rows).}
\end{table}
...
\midrule
Phishing (0) & 0.93 & 0.92 & 0.93 & 500 \\
Valid (1)    & 0.92 & 0.94 & 0.93 & 500 \\
\midrule
\textbf{Accuracy} & \multicolumn{4}{r}{\textbf{0.93} (1{,}000 / 1{,}000)} \\
\bottomrule
\end{tabular}
\caption{Emails label predictor evaluated \emph{independently}, directly on
ground-truth concepts (bypassing the concept extractor), on the 1{,}000-row
test set -- the upper bound on task accuracy given perfect concept
predictions. (Train-set accuracy 0.95, validation-set accuracy 0.97, not
shown in full.)}
\end{table}

Since \code{fit\_intercept=False}, the fitted weight vector alone is
directly interpretable as each concept's signed contribution to the
``valid email'' decision, with no additive bias term to account for:

\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{emails/figures/label_predictor_weights.pdf}
\caption{Emails label predictor: learned weight per concept. Positive
weights push the decision toward ``Valid''; negative weights push toward
``Phishing''. Urgency, Fear+Authority, and Open attachment are the three
concepts most associated with phishing; Neutral, Curiosity, and Reply are
most associated with valid email.}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{CUB concept extractor: per-concept classification report on the
sparrow pair's 59 held-out test images (multi-label; support is the number
of positive instances per concept out of 59 images, not 59 itself). Note
the extractor was never trained on either sparrow species
(Section~\ref{sec:cub-restriction}).}
\end{table}
...
label_model = LogisticRegression(
    max_iter=1000, class_weight='balanced', fit_intercept=False
)
label_model.fit(train_pair_concepts_pm1, train_pair_y)
\end{lstlisting}

As with the emails label predictor, \code{fit\_intercept=False} is used so
the fitted weight vector alone is directly interpretable as each concept's
signed contribution, with no additive bias term. Unlike emails,
\code{penalty=None} is \emph{not} set here -- default L2 regularization
($C=1.0$) is kept, though with only two well-separated classes and 6
ground-truth concepts the fitted model reaches perfect training-time
separability regardless of regularization strength (see below).
Empirically, this choice is a no-op: refitting with \code{fit\_intercept=True}
instead converges to an intercept of $-2.97\times10^{-17}$ (i.e.\ effectively
zero already) and produces bit-identical weights and predictions, since the
sparrow-pair task is symmetric enough that the optimizer never finds a
class-prior offset useful.
...
Class & Precision & Recall & F1 & Support \\
\midrule
Le Conte's Sparrow (123) & 1.00 & 1.00 & 1.00 & 29 \\
Savannah Sparrow (126)   & 1.00 & 1.00 & 1.00 & 30 \\
\midrule
\textbf{Accuracy} & \multicolumn{4}{r}{\textbf{1.00} (59 / 59)} \\
\bottomrule
\end{tabular}
\caption{CUB label predictor evaluated \emph{independently}, directly on
ground-truth concepts (bypassing the concept extractor), on the sparrow
pair's 59 test images -- the upper bound on task accuracy given perfect
concept predictions: the six chosen concepts perfectly separate the two
species when known exactly.}
\end{table}

\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{cub/figures/label_predictor_weights.pdf}
\caption{CUB label predictor: learned weight per concept (sparrow pair).
Positive weights push the decision toward Savannah Sparrow; negative
weights push toward Le Conte's Sparrow. The perfectly symmetric magnitudes
($\pm0.71$) reflect the six concepts' perfect linear separability of the
two species (Table above).}
\end{figure}
...
Comparing this against the independently-trained label predictor's 100\%
upper bound (previous subsection): the end-to-end accuracy gap (81.4\%
vs.\ 100\%) is attributable entirely to concept-extraction error on
out-of-training-distribution species, not to the label predictor's
decision boundary.
...
\begin{itemize}[leftmargin=*]
    ...
    predictor always consumes $\tanh$ of the concept SVMs'
    \code{decision\_function} output, not their binarized $\{0,1\}$
    predictions -- a continuous, signed confidence score in $(-1, 1)$ for
    each concept.
